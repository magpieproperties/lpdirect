...
let elements = Array.from(document.querySelectorAll(sel));
  return elements.length;
  	...
  	for (let i = 0; i <= boxNumbers ; i++) 
	{
		...
		var name = {'firstname':OwnerFirstName,'lastname':OwnerLastName,'full_legal':legal_result,'lot_number':lotNumber};
		namesData.push(name);
		//console.log(name);
			...
			for(var r = 0; r < namesData.length; r++)
			{
				...
				if(legalData.includes('LT'))
				{
					...
					// 	await page.waitFor(1000);
						...
						if(tableRowCnt == 0)
						{
							var json = {'FirstName':item.firstname,'LastName':item.lastname,'County':'Lake'};
...
var smtpTransport = nodemailer.createTransport({
	...
	var mailOptions = {
	  ...
	  subject: "Lake Count LP Mailer list No Results",
...
var mailOptions = {
  ...
  subject: "Lake LP",
  generateTextFromHTML: true,
  html: "<b>Lake County LIS PENDONS From the machines!</b>",
